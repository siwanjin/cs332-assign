...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    val leftOpt = try Some(left.mostRetweeted) catch { case _: NoSuchElementException => None }
    val rightOpt = try Some(right.mostRetweeted) catch { case _: NoSuchElementException => None }
    val candidates = List(Some(elem), leftOpt, rightOpt).flatten
