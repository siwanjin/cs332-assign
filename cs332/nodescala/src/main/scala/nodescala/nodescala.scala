...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    try {
      val it = response
      while (token.nonCancelled && it.hasNext) {
        exchange.write(it.next())
      }
    } finally {
      // Always close the exchange to signal end-of-response,
      // even if cancellation happened in the middle.
      exchange.close()
    }
  }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    def loop(ct: CancellationToken): Future[Unit] = {
      if (!ct.nonCancelled) Future.unit
      else listener.nextRequest().flatMap { case (req, xchg) =>
        Future { respond(xchg, ct, handler(req)) }
        loop(ct)
      }
    }
    val serverSub = Future.run() { ct => loop(ct) }
...
object NodeScala {
  ...
  object Exchange {
    def apply(exchange: HttpExchange): Exchange = new Exchange {
      ...
      def request: Request = {
        val headers = for ((k, vs) <- exchange.getRequestHeaders.asScala)
          yield (k, vs.asScala.toList)
