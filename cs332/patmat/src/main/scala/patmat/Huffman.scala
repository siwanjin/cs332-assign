...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = 
    chars.groupBy(identity).map { case (c, l) => (c, l.size) }.toList
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case t1 :: t2 :: rest =>
      (makeCodeTree(t1, t2) :: rest).sortBy(weight)
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def loop(sub: CodeTree, rest: List[Bit]): List[Char] = sub match {
      case Leaf(c, _) =>
        if (rest.isEmpty) List(c)
        else c :: loop(tree, rest)
      case Fork(l, r, _, _) =>
        rest match {
          case Nil => Nil
          case 0 :: tail => loop(l, tail)
          case 1 :: tail => loop(r, tail)
          case _         => Nil
        }
    }
    loop(tree, bits)
  ...
  def convert(tree: CodeTree): CodeTable = {
    def loop(t: CodeTree, prefix: List[Bit]): CodeTable = t match {
      case Leaf(c, _) => List((c, prefix))
      case Fork(l, r, _, _) =>
        mergeCodeTables(loop(l, prefix :+ 0), loop(r, prefix :+ 1))
    }
    loop(tree, Nil)
  }
